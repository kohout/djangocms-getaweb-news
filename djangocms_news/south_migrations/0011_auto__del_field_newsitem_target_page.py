...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'NewsItem.target_page'
        db.delete_column(u'djangocms_news_newsitem', 'target_page_id')

        # Adding M2M table for field target_page on 'NewsItem'
        m2m_table_name = db.shorten_name(u'djangocms_news_newsitem_target_page')
        ...
        db.create_unique(m2m_table_name, ['newsitem_id', 'page_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'NewsItem.target_page'
        raise RuntimeError("Cannot reverse this migration. 'NewsItem.target_page' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'NewsItem.target_page'
        db.add_column(u'djangocms_news_newsitem', 'target_page',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['cms.Page']),
                      keep_default=False)

        # Removing M2M table for field target_page on 'NewsItem'
        db.delete_table(db.shorten_name(u'djangocms_news_newsitem_target_page'))
